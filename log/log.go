...
// All log.Normal messages can be filtered out with regexp `^chcontainer\:\s`.
package log
...
func SetupLog() func() {
	...
	if viper.IsSet("debug") {
		...
		debug = strings.ToLower(os.Getenv("CHC_DEBUG")) == "true"
	}
	if viper.IsSet("quiet") {
		quiet = viper.GetBool("quiet")
	} else {
		quiet = strings.ToLower(os.Getenv("CHC_QUIET")) == "true"
	}
	if viper.IsSet("log") {
		info = viper.GetBool("log")
	} else {
		info = strings.ToLower(os.Getenv("CHC_LOG")) == "true"
	}

	if debug || info {
		logFileOnce.Do(func() {
			var err error
			logFile, err = os.OpenFile("chcontainer.log", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
			if err != nil {
				log.Panic(err)
			}
		})
	...
	if quiet {
		...
		Normal = log.New(stderrWriter, "chcontainer: ", 0)
	}
